...
class SDMXWrapper(object):
    ...
    def dimensions(self):
        return [(k.lower(), label, v.local_repr.enum.id if v.local_repr.enum else None)
                for k, v in self.dsd.dimensions.items() if k != 'TIME_PERIOD'
                for lang, label in v.concept.name.items() if lang == 'en']

    def attributes(self):
        return [(k.lower(), v.local_repr.enum.id if v.local_repr and v.local_repr.enum else None)
    ...
    def data(self):
        ...
        def handle_serie(serie):
            keys = {k.lower(): v for k,v in serie.key._asdict().items()}
            return [{
                **keys,
                **{k.lower(): v for k, v in o._asdict().items() if k != 'attrib'},
                **{k.lower(): v for k, v in o['attrib']._asdict().items()}}
